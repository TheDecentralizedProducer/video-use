...
# -------- ffmpeg binary ------------------------------------------------------
# Prefer ffmpeg-full (has libass + ProRes 4444 alpha decoding) over system ffmpeg.
import shutil as _shutil
FFMPEG = (
    "/opt/homebrew/opt/ffmpeg-full/bin/ffmpeg"
    if _shutil.which("/opt/homebrew/opt/ffmpeg-full/bin/ffmpeg")
    else "ffmpeg"
)
...
def build_final_composite(
    ...
    for idx, ov in enumerate(overlays, start=1):
        ...
        next_label = f"[v{idx}]"
        filter_parts.append(
            f"{current}[a{idx}]overlay=enable='between(t,{t:.3f},{end:.3f})':format=auto{next_label}"
        )
        current = next_label
